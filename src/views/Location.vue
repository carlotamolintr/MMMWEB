<template>
  <v-container xs12 sm5 md5 lg4 xl4 fluid fill-height justify-center>
    <v-card sm5 md5 lg4 xl4 class="card-Box">
      <v-flex xs12 class="pa-4">
        <h1>Locate your sighting</h1>
        <v-flex class="d-flex">
          <v-btn color="blue" v-on:click="gpsLocation()" class="justify-center">add gps location</v-btn>
        </v-flex>
        <v-text-field v-show="location == true" label="Latitude" readonly class="latLonBox" v-model="lat">
        </v-text-field>
        <v-text-field v-show="location == true" label="Longitude" readonly class="latLonBox" v-model="lon">
        </v-text-field>
        <p>{{error}}</p>
      ...
      <v-flex xs12>
        <v-text-field xs12 v-model="numberIndividuals" :items="numberIndividuals" :counter="3"
          label="Number Individuals" required class="pa-4" type="number"></v-text-field>
      </v-flex>
      <v-flex xs12>
      ...
      <v-menu v-model="menu2" :close-on-content-click="false" :nudge-right="40" lazy transition="scale-transition"
        ...
        <template v-slot:activator="{ on }">
          <v-text-field v-model="date" label="Picker without buttons" prepend-icon="event" readonly v-on="on">
          </v-text-field>
        </template>
