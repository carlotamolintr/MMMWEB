    ...
    <v-card class="ma-2 my-4 semi-transparent" v-for="(specie, index) in searchField" :key="specie.AcceptedCommonName">
      ...
      <v-card-title primary-title class="moreOpacity">
        ...
        <v-flex v-show="show == index" style="width: 100%">
          <v-flex xs12 sm6>
            ...
            <div v-show="specie.AlternateCommonName !== undefined">
      ...
      <v-card-actions class="moreOpacity">
        ...
        <v-btn icon @click="eyeCheck(specie, index)">
          <v-icon :class="{
              'blue--text': specie.fav == true,
              'grey--text': !specie.fav || specie.fav == false,
            }" flat>
            {{
              specie.fav != true ? "remove_red_eye" : "remove_red_eye"
            }}</v-icon>
        </v-btn>
        <v-spacer></v-spacer>
        <!-- <v-spacer></v-spacer> -->
        <v-btn icon @click="arrow(index)">
          <v-icon>{{
            show != index ? "keyboard_arrow_down" : "keyboard_arrow_up"
          }}</v-icon>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      // dataSpecies() {
      //   return todasSpecies; // getters para coger la información del storage. DataSpecies actua ahora como una variable.
      // },
    },
  };
</script>
